...
|---|---|
| `./sync.sh` | 安装或更新 |
| `./sync.sh --dry-run` | 只预告将要发生什么，不写任何东西 |
| `./sync.sh --status` | 只报告当前装了什么版本、什么时候装的、对应哪个提交 |
| `./sync.sh --rollback` | 回滚到最近一次备份 |
| `./sync.sh --force` | 绕过「已是最新」和「这是降级」两道拦截，强制安装 |

`DSH_HOME` 默认 `~/.dsh`，装在别的路径就 `DSH_HOME=/opt/dsh ./sync.sh`。

### 版本闸门（防止静默降级）

安装时会在 preset 目录里写一份 `INSTALLED.json`：

```json
{ "preset": "paper", "version": "0.1.3", "commit": "9262a0b",
  "installedAt": "2026-09-15T06:02:11Z", "installedBy": "dsh", "dshHome": "/home/dsh/.dsh" }
```
...
|---|---|
| 尚未安装 | 正常安装 |
| 版本更新，如 `0.1.2 → 0.1.3` | 正常安装，打印版本变化 |
| 版本相同 | **拒绝**，提示已是最新；要重装得 `--force` |
| **版本倒退**，如 `0.1.3 → 0.1.0` | **报错退出**，提示可能是切错了提交/分支；要降级得 `--force` |

没有这道闸门时，客户拉错分支或仓库被 force-push 回旧状态，会**静默把客户降级**——这是实测确认过的行为。

支持客户时，一句 `./sync.sh --status` 就知道对方跑的是哪一版。
